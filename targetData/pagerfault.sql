...
SELECT * FROM t1 
UPDATE t2 SET x = a_string(280) 
UPDATE t1 SET x = a_string(280) WHERE rowid = 5 
PRAGMA journal_mode = PERSIST;
    BEGIN;
      CREATE TABLE t1(x, y UNIQUE);
      INSERT INTO t1 VALUES(a_string(333), a_string(444));
...
UPDATE t2 SET a = a_string(800), b = a_string(800) 
ROLLBACK 
...
file:one?mode=memory&cache=shared
CREATE TABLE t1(x);
